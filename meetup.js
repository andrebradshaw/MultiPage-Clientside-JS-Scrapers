...
function getApprxLastVisit(elm) {  var oneday = 86400000;  var str = reg(/(?<=Last visited ).+/.exec(elm), 0);  var now = new Date().getTime();  if (/today/.test(str) === true) {    return new Date(now - (oneday));  }  if (/past few days/.test(str) === true) {    return new Date(now - (oneday * 3));  }  if (/a week ago/.test(str) === true) {    return new Date(now - (oneday * 7));  }  if (/past two weeks/.test(str) === true) {    return new Date(now - (oneday * 14));  }  if (/over two weeks ago/.test(str) === true) {    return new Date(now - (oneday * 21));  }  if (/over a month ago/.test(str) === true) {    return new Date(now - (oneday * 40));  }  if (/over two months ago/.test(str) === true) {    return new Date(now - (oneday * 70));  }  if (/over three months ago/.test(str) === true) {    return new Date(now - (oneday * 100));  }  if (/over four months ago/.test(str) === true) {    return new Date(now - (oneday * 130));  }  if (/over five months ago/.test(str) === true) {    return new Date(now - (oneday * 160));  }  if (/over six months ago/.test(str) === true) {    return new Date(now - (oneday * 190));  }  if (/over seven months ago/.test(str) === true) {    return new Date(now - (oneday * 220));  }  if (/over seven months ago/.test(str) === true) {    return new Date(now - (oneday * 220));  }  if (/over eight months ago/.test(str) === true) {    return new Date(now - (oneday * 250));  }  if (/over nine months ago/.test(str) === true) {    return new Date(now - (oneday * 280));  }  if (/over ten months ago/.test(str) === true) {    return new Date(now - (oneday * 310));  }  if (/over eleven months ago/.test(str) === true) {    return new Date(now - (oneday * 340));  }  if (/over a year ago/.test(str) === true) {    return new Date(now - (oneday * 370));  }}
...
function getProfileData(links, t) {
  setTimeout(() => {
    function getProfile(link, n, end) {
      setTimeout(() => {
        ...
        if (n > (end - 2)) {
          dl(reg(/(?<=meetup\.com\/).+?(?=\/)/.exec(window.location.href), 0) + '_members.json', JSON.stringify(memberDetailsArray));
        }

      }, ((n + 1) * 5000));
    }
    for (y = 0; y < links.length; y++) {
      getProfile(links[y], y, links.length);
    }
...
function dl(filename, text) {
  setTimeout(() => {
    console.log('running profile pages');
...
runfirst.then(getProfileData(memberLinksArray, memberScrape_waitTime));
